...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 12 19:18:01 2019
...
if __name__ == "__main__":
    ...
    s = ['audiobooks', 'broadcast_interview', 'child', 'clinical', 'court', 'maptask', 'meeting', 'restaurant', 'socio_field', 'socio_lab', 'webvideo']
    X=data['devIVs']
    X=X.transpose()
    labels=data['labels']
    labels=labels[:,0]
    ...
    for i in np.unique(labels):
        mask = labels == i
        plt.scatter(Y[mask, 0], Y[mask, 1], 20, label=s[i-1], c=colors[i-1])
    ax.legend()
